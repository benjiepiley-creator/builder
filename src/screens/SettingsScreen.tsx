...
export const SettingsScreen = (_props: Props) => {
  const { userPlan, scansThisMonth, showUpgrade } = useSubscriptionStore();
  ...
  return (
    <ScreenContainer>
      <View>
        <HeaderBar showBack onBack={() => _props.navigation.goBack()} eyebrow="Settings" title={APP_NAME} />
        ...
        <GlassCard glow="cyan">
          ...
          <Text className="mt-1 text-slate-400">
            {PLAN_CONFIG[userPlan].monthlyScanLimit
              ? `${scansThisMonth}/${PLAN_CONFIG[userPlan].monthlyScanLimit} free scans used this month`
        ...
        <View className="mt-6 gap-3">
          <SettingRow label="Supabase" value={isSupabaseConfigured ? "Env ready, local active" : "Local-only mode"} />
